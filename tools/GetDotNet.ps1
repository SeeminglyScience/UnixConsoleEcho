...
param(
    ...
    [switch] $Unix = $IsLinux -or $IsMacOS
)
end {
    ...
    if ($Unix.IsPresent) {
        $uri = "https://raw.githubusercontent.com/dotnet/cli/v$Version/scripts/obtain/dotnet-install.sh"
        ...
        $uri = "https://raw.githubusercontent.com/dotnet/cli/v$Version/scripts/obtain/dotnet-install.ps1"
        $scriptText = [System.Net.WebClient]::new().DownloadString($uri)
